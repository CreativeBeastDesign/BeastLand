...
import { customerName, formatMoney } from "../data/format.js";
import { docTypeLabels } from "../data/types.js";
import { fieldsAt } from "../data/views.js";
...
import { runSet } from "../tiling/workspace-commands.js";
...
function say(ctx, before, id, after = "") {
    ctx.print([{ text: before }, ...idSpans(id), { text: after }], "output");
}
function printAmbiguous(ids, ctx) {
...
function resolveProject(token, ctx) {
...
function resolveDocument(token, ctx) {
    const resolved = kinds.resolve(token);
    if (!resolved) {
        ctx.print(`unknown id: ${token}`, "error");
        return null;
...
function requireProject(id, ctx) {
 ...
 */
function printRecord(ctx, id, record, defs, level) {
 ...
 */
function printList(ctx, records, defs, level, empty) {
    if (records.length === 0) {
...
// list / ls
// ---------------------------------------------------------------------------
function printProjectList(ctx, level) {
...
function applyProjectSet(container, args, ctx) {
...
// ---------------------------------------------------------------------------
function printProjectDocuments(id, ctx) {
    if (!requireProject(id, ctx))
        return;
...
function doLink(projectId, args, ctx) {
    if (!requireProject(projectId, ctx))
        return;
    const token = args[0];
    if (!token) {
...
function doUnlink(projectId, args, ctx) {
    if (!requireProject(projectId, ctx))
        return;
    const token = args[0];
    if (!token) {
...
function printProjectLog(id, ctx) {
    if (!requireProject(id, ctx))
        return;
...
export const projectCommand = {
    ...
    usage: "project [list [-d|-f]] | project new --name/-n --customer --status --start --end --budget --description | " +
    ...
    complete: (args) => {
        ...
        if (args.length === 2 && args[0].startsWith("#"))
            return idSubcommandSuggestions;

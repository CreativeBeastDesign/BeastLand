...
export const projectStatusValues = ["planned", "active", "on_hold", "done"];
export const projectFieldFlags = [
...
export function projectFieldsFromFlags(parsed) {
    const fields = {};
    const name = flag(parsed, "name", "n");
    if (typeof name === "string")
        fields.name = name;
    const status = flag(parsed, "status");
    if (typeof status === "string" && projectStatusValues.includes(status)) {

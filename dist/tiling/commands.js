...
import { flag, parseArgs, } from "../shell/commands.js";
import { containerCommands, describePatch, detailFlags, levelFromArgs, printAmbiguous, printList, printTable, runSet, say, showRecord, sid, spawnSizeFromArgs, } from "./workspace-commands.js";
export { runSet } from "./workspace-commands.js";
import { customerName, formatMoney, itemLabels, itemTotal, resolveItemRef, splitName, } from "../data/format.js";
import { docTypeLabels } from "../data/types.js";
import { customerFields, documentFields } from "../data/views.js";
import { kinds } from "./kinds.svelte.js";
import { customerFieldFlags, customerFieldsFromFlags, documentFieldFlags } from "../data/fields.js";
import { notify } from "../shell/toasts.svelte.js";
...
import { projects } from "../project/store.svelte.js";
/** Resolve `#xp`-style input to a record of the expected kind, printing errors. */
function resolveKind(token, kind, ctx) {
...
// customer
// ---------------------------------------------------------------------------
function printCustomerList(ctx, level) {
...
function applyCustomerSet(container, args, ctx) {
...
// docs
// ---------------------------------------------------------------------------
function printDocList(ctx, level) {
    printList(ctx, data.documents, documentFields, level, "(no documents)");
}
function applyDocumentSet(container, args, ctx) {
...
const docsCommand = {
    ...
    run: (args, ctx) => {
        ...
        if (head === "new") {
            ...
            let docType = "offer";
            if (flag(parsed, "invoice", "i") !== undefined)
                docType = "invoice";
            else if (flag(parsed, "confirmation", "c") !== undefined)
...
// item
// ---------------------------------------------------------------------------
function itemFieldsFromFlags(parsed) {
    const fields = {};
    ...
    if (typeof price === "string")
        fields.unitPriceMinor = Math.round(parseFloat(price) * 100);
    const qty = flag(parsed, "qty", "n");
    if (typeof qty === "string")
        fields.quantity = parseFloat(qty);
    const description = flag(parsed, "description");
    if (typeof description === "string")
        fields.description = description;
    if (flag(parsed, "optional", "o") !== undefined)
        fields.isOptional = true;
    if (flag(parsed, "required") !== undefined)
        fields.isOptional = false;
    return fields;
}
function printItems(items, ctx) {
 ...
 * `$lib/data/kinds.ts`).
 */
export function handleItem(documentId, args, ctx) {
    ...
    if (sub === "set") {
        ...
        if (Object.keys(patch).length === 0) {
            ctx.print("usage: item <n|a|last> set --title/-t|--price/-p|--qty/-n|--optional|--required|--description <value>", "error");
            return;
        }
...
const itemCommand = {
    ...
    complete: (args) => {
        ...
        if (args.length === 1) {
            ...
            const suggestions = doc.items.map((item, i) => ({
                value: labels[i],
                label: item.title,
...
export const tilingCommands = [customerCommand, docsCommand, itemCommand, ...containerCommands, resetCommand];

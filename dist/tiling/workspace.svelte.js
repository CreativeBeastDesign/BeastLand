...
import { undoStack } from "../shell/undo.svelte.js";
import { DEFAULT_SIZE, GRID_COLUMNS, MIN_SIZE, } from "./types.js";
...
function readPersisted() {
    ...
    if (current)
        return { persisted: current, migrated: false };
    const old = readOld();
    if (old)
        return { persisted: old, migrated: true };
...
function createWorkspace() {
    ...
    function findByContent(contentId) {
    ...
    function findFreeRect(w, h) {
        ...
        for (let y = 0; y <= maxY; y++) {
            for (let x = 0; x <= GRID_COLUMNS - w; x++) {
                ...
                if (!containers.some((c) => overlaps(candidate, c)))
                    return { x, y };
            }
        }
    ...
    function peekSpawnFor(kind) {
        return peekSpawn(kinds.sizeOf(kind));
    }
    function spawn(kind, contentId, opts) {
    ...
    function pushSpawnUndo(container) {
        if (applyingUndo)
            return;
     ...
     */
    function restoreContainer(snapshot) {
        applyingUndo = true;
        try {
            const layout = activeLayout();
    ...
    function close(id) {
        ...
        if (selectedId === id) {
            selectedId =
                containers.length === 0
                    ? null
                    : containers.reduce((nearest, c) => Math.abs(c.id - id) < Math.abs(nearest.id - id) ? c : nearest).id;
        }
    ...
    function selectDirection(dir) {
        ...
        for (const c of layout.containers) {
            ...
            const dx = ox - cx;
            const dy = oy - cy;
            const inDirection = dir === "up" ? dy < 0 : dir === "down" ? dy > 0 : dir === "left" ? dx < 0 : dx > 0;
            if (!inDirection)
                continue;
    ...
    function peekMove(id, dir) {
        ...
        const target = { x: container.x + dx, y: container.y + dy, w: container.w, h: container.h };
    ...
    function pushRectUndo(label, restores) {
        if (applyingUndo)
            return;
        undoStack.push({
            label,
    ...
    function move(id, dir) {
        ...
        if (!peek.ok)
            return { ok: false, reason: peek.reason ?? "cannot move" };
        const layout = activeLayout();
        const container = get(id);
        ...
        if (peek.swapWith !== undefined) {
            ...
            const containers = layout.containers.map((c) => {
                ...
                if (c.id === other.id)
                    return { ...c, x: container.x, y: container.y };
                return c;
            });
            updateActive({ containers });
            persist();
    ...
    function peekResize(id, size) {
        ...
        const target = { x: container.x, y: container.y, w, h };
    ...
    function resize(id, size) {
        ...
        if (!peek.ok)
            return { ok: false, reason: peek.reason ?? "cannot resize" };
        const container = get(id);
    ...
    function resolveLayout(idOrIndex) {
    ...
    function switchLayout(idOrIndex) {
    ...
    function create(name) {
        ...
        const layout = emptyLayout(nextLayoutId(), trimmed || nextAutoName());
    ...
    function remove(id) {
        if (layouts.length <= 1)
            return false; // never remove the last workspace

...
import { clearHistory } from "../shell/history.js";
import { kinds } from "./kinds.svelte.js";
import { fieldsAt } from "./views.js";
import { flag, parseArgs, } from "../shell/protocol.js";
// ---------------------------------------------------------------------------
...
export function padSpans(spans, width) {
...
export function say(ctx, before, id, after = "") {
    ctx.print([{ text: before }, ...idSpans(id), { text: after }], "output");
}
export function printAmbiguous(ids, ctx) {
...
export function levelFromArgs(parsed) {
    if (flag(parsed, "full", "f") !== undefined)
        return "full";
    if (flag(parsed, "details", "d") !== undefined)
        return "details";
    return null;
}
/**
 ...
 */
export function printRows(ctx, id, rows, level) {
    if (level === "list") {
...
export function printRecord(ctx, id, record, defs, level) {
    printRows(ctx, id, fieldsAt(defs, level).map((f) => ({ label: f.label, value: f.get(record) })), level);
}
/**
 ...
 */
export function printList(ctx, records, defs, level, empty) {
    if (records.length === 0) {
    ...
    records.forEach((r, ri) => {
        const spans = [...padSpans(idSpans(r.id), idWidth), { text: "  " }];
...
export function showRecord(ctx, kind, id, level) {
...
export function printTable(ctx, rows) {
    if (rows.length === 0)
        return;
    const widths = [];
...
export function containerSuggestions() {
...
export function recordSuggestions(kind) {
    const specs = kind ? [kinds.get(kind)].filter((s) => s !== undefined) : kinds.all;
    return specs.flatMap((spec) => (spec.ids?.() ?? []).map((id) => ({
 ...
 */
export function runSet(container, args, ctx) {
...
// ---------------------------------------------------------------------------
export function directionFromArgs(parsed) {
    if (flag(parsed, "up", "u") !== undefined)
        return "up";
    if (flag(parsed, "down", "d") !== undefined)
        return "down";
    if (flag(parsed, "left", "l") !== undefined)
        return "left";
    if (flag(parsed, "right", "r") !== undefined)
        return "right";
    return undefined;
}
function doMove(container, args, ctx) {
...
export function applyContainerArgs(container, args, ctx) {
    if (args.length === 0)
        return;
    ...
    if (action)
        return action.run(container.contentId, rest, ctx);
    ...
    if (width !== undefined || height !== undefined) {
        ...
        if (typeof width === "string")
            size.w = parseInt(width, 10);
        if (typeof height === "string")
            size.h = parseInt(height, 10);
 ...
 */
export function spawnSizeFromArgs(args) {
    const parsed = parseArgs(args);
    const size = {};
    const w = flag(parsed, "width", "w");
    const h = flag(parsed, "height", "h");
    if (typeof w === "string" && !Number.isNaN(parseInt(w, 10)))
        size.w = parseInt(w, 10);
    if (typeof h === "string" && !Number.isNaN(parseInt(h, 10)))
        size.h = parseInt(h, 10);
    const rest = [];
...
function resizeIntent(container, widthFlag, heightFlag, target) {
 ...
 */
export function previewContainerArgs(container, args) {
    const target = `@${container.id}`;
    if (args.length === 0)
        return { target };
    const [head, ...tail] = args;
    if (head === "close")
        return { target, hint: "close" };
    if (head === "title")
        return { target, hint: "title" };
    if (head === "move") {
    ...
    const dir = directionFromArgs(parsed);
    if (dir)
        return moveIntent(container, dir, target);
    const width = flag(parsed, "width", "w");
    const height = flag(parsed, "height", "h");
    if (width !== undefined || height !== undefined) {
 ...
 */
function containerFlagsFor(kindOf) {
    return (args) => {
        const verb = args[1];
        if (!verb)
            return null;
...
export function actionSuggestions(kind) {
    if (!kind)
        return [];
    return (kinds.get(kind)?.actions ?? []).map((a) => ({ value: a.name, description: a.description, kind: "subcommand" }));
}
/**
 ...
 */
export function actionFlagsFor(spec, args, base = detailFlags) {
 ...
 */
export async function runKindAction(spec, contentId, verb, args, ctx) {
    const action = spec.actions?.find((a) => a.name === verb);
    if (!action)
        return false;
...
const atCommand = {
    ...
    complete: (args) => {
        ...
        if (args.length === 2)
            return actionSuggestions(kindOfAt(args[0]));
        return [];
    },
    run: (args, ctx) => {
    ...
    preview: (args) => {
        ...
        const container = workspace.get(id);
        if (!container)
            return { target: `@${id}`, invalid: true, hint: `no container @${id}` };
...
const hashCommand = {
    ...
    complete: (args) => {
        ...
        if (args.length === 2)
            return actionSuggestions(kindOfHash(args[0]));
        return [];
    },
    run: (args, ctx) => {
        const [token, ...rest] = args;
    ...
    preview: (args) => {
        ...
        if (levelFromArgs(parseArgs(rest)))
            return { target: sid(resolved.id), hint: "print" };
...
const wsCommand = {
    ...
    preview: (args) => {
        const [head] = args;
        if (!head || wsVerbs.has(head))
            return null;
        const target = resolveLayout(head);
        if (!target)
            return { target: "ws", hint: `no such workspace: ${head}`, invalid: true };
        return { target: "ws", hint: `switch → ${target.name}` };
    },
};
/** The record-agnostic container commands: `@n`, `#id`, `ls`, `close`, `ws`. */
export const containerCommands = [atCommand, hashCommand, lsCommand, closeCommand, wsCommand];
/** `containerCommands` plus `reset --layout` — everything a workspace needs without any data slice. */
export const workspaceCommands = [...containerCommands, layoutResetCommand];

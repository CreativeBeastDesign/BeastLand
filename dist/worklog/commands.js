...
import { itemLabels, resolveItemRef, formatDate } from "../data/format.js";
import { flag, parseArgs, } from "../shell/commands.js";
import { worklog, clock } from "./store.svelte.js";
import { entryMinutes, formatDuration, dayKey, timeOfDay } from "./types.js";
...
function itemLabelFor(entry) {
    if (!entry.documentId || entry.itemIndex === null)
        return null;
...
function entryRefSpans(entry) {
    ...
    if (entry.projectId)
        return idSpans(entry.projectId);
    return [];
}
 ...
 */
function parseLogTarget(positional, ctx) {
    let rest = positional;
    let documentId = null;
    let projectId = null;
...
function rangeFromFlags(parsed) {
    if (flag(parsed, "all", "a") !== undefined)
        return undefined;
    ...
    if (flag(parsed, "day", "d") !== undefined)
...
let lastPrinted = [];
function printTimeline(args, ctx) {
...
function doStart(args, ctx) {
    ...
    const refSpans = entryRefSpans(entry);
    const spans = [{ text: "started" }];
    if (refSpans.length > 0)
        spans.push({ text: " " }, ...refSpans);
    if (entry.note)
        spans.push({ text: `${refSpans.length > 0 ? "  " : " "}${entry.note}` });
...
function doAdd(args, ctx) {
    ...
    const refSpans = entryRefSpans(entry);
    const spans = [{ text: "added " }, ...refSpans];
    if (entry.note)
        spans.push({ text: `${refSpans.length > 0 ? "  " : ""}${entry.note}` });
...
function targetSuggestions() {
    ...
    const projects = kinds
        .get("project")
        ?.ids?.()
        ?.map((id) => ({ value: sid(id), label: kinds.labelOf("project", id), kind: "value" })) ?? [];
    return [...documents, ...projects];
}
function itemSuggestionsFor(documentId) {
    const doc = data.getDocument(documentId);
    if (!doc)
        return [];
...
export const logCommand = {
    ...
    usage: 'log [ls|list [-w|-d|-a]] | log start [#id] [<n|a|last>] ["note"] | log stop | log status | ' +
    ...
    preview: (args) => {
        ...
        if (head === "start") {
            ...
            if (existing)
                return { target: `@${existing.id}`, hint: "start" };
...
export const worklogCommands = [logCommand];

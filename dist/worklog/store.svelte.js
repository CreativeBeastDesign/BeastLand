...
function seedEntries() {
    ...
    const mk = (id, documentId, itemIndex, note, startedAt, stoppedAt) => ({
        id: `worklog:${id}`,
        documentId,
        itemIndex,
    ...
    return [
        ...
        mk("seedaaaaaaaaaaaaaaaa6", SEED_DOC_1, 2, "Kalibrierung", new Date(now - 25 * 60000).toISOString(), null),
    ];
}
...
function createWorklog() {
    ...
    function stop() {
        ...
        if (!running)
            return null;
        const now = new Date().toISOString();
        const updated = { ...running, stoppedAt: now, updatedAt: now };
    ...
    function minutesFor(documentId, itemIndex) {
        return entries

...
const cache = new WeakMap<readonly Command[], CommandIndex>();

/** Build (or reuse) the lookup index for a commands array. */
export function indexCommands(commands: readonly Command[]): CommandIndex {
  const cached = cache.get(commands);
  if (cached) return cached;

  const byName = new Map<string, Command>();
  const matchers: Command[] = [];
  for (const c of commands) {
    ...
    if (c.match) matchers.push(c);
  }

  const index: CommandIndex = { byName, matchers };

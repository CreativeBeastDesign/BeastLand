...
export function initHistoryNav(): HistoryNavState {
...
function entryAt(state: HistoryNavState, history: string[], index: number): string {
...
export function navigateUp(
  ...
  if (history.length === 0) return { state, text: liveText, caret: liveCaret };

  if (state.index === null) {
    const index = history.length - 1;
    const next: HistoryNavState = { ...state, index, draftText: liveText, draftCaret: liveCaret };
    const text = entryAt(next, history, index);
    return { state: next, text, caret: text.length };
  }

  if (state.index === 0) return { state, text: entryAt(state, history, 0), caret: liveCaret };

  const index = state.index - 1;
  const next: HistoryNavState = { ...state, index };
  const text = entryAt(next, history, index);
  return { state: next, text, caret: text.length };
}
 ...
 */
export function navigateDown(state: HistoryNavState, history: string[], currentText: string, currentCaret: number): HistoryNavResult {
  if (state.index === null) return { state, text: currentText, caret: currentCaret };
  ...
  if (currentText !== history[state.index]) edits.set(state.index, currentText);
  else edits.delete(state.index);

  const index = state.index + 1;
  if (index >= history.length) {
    const next: HistoryNavState = { index: null, draftText: "", draftCaret: 0, edits };
    return { state: next, text: state.draftText, caret: state.draftCaret };
  }

  const next: HistoryNavState = { ...state, index, edits };
  const text = entryAt(next, history, index);
  return { state: next, text, caret: text.length };
}
